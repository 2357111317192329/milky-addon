...
public class Utils
{
    public static int NbtgetInt(CompoundTag tag, String key, int fallback) {
        Tag e = tag.get(key);
        if (e instanceof IntTag nbtInt) return nbtInt.intValue();
        return fallback;
    }
    public static boolean NbtgetBoolean(CompoundTag tag, String key, boolean fallback) {
        Tag e = tag.get(key);
        if (e instanceof ByteTag b) return b.byteValue() != 0;
        return fallback;
    }
    public static float NbtgetFloat(CompoundTag tag, String key, float fallback) {
        Tag e = tag.get(key);
        if (e instanceof FloatTag f) return f.floatValue();
        return fallback;
    }
    public static double NbtgetDouble(CompoundTag tag, String key, double fallback) {
        Tag e = tag.get(key);
        if (e instanceof DoubleTag d) return d.doubleValue();
        return fallback;
    }
    public static String NbtString(CompoundTag tag, String key, String fallback) {
        Tag e = tag.get(key);
        if (e instanceof StringTag s) return s.value();
        return fallback;
    }
